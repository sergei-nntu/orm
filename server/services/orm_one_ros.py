    ...
    while True:
        if scheduled_trajectory is not None:
            if current_trajectory != scheduled_trajectory:
                ...
                apply_trajectory(
                    current_trajectory.joint_names, current_trajectory.points
                )
        time.sleep(0.1)
...
def joint_trajectory_callback(msg):
    ...
    apply_trajectory(
        msg.trajectory[0].joint_trajectory.joint_names,
...
rospy.Subscriber(
    "/move_group/display_planned_path", DisplayTrajectory, joint_trajectory_callback
)
